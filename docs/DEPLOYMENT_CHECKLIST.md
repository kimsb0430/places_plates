...
- 운영 PostgreSQL에서 V1~V15 이력, 애플리케이션 테이블 14개, 세션 테이블 2개와 `FORCE ROW LEVEL SECURITY`가 13개 개인 데이터 테이블에 적용됐는지 확인한다. V11 적용 후 완료 작업·검사 통과 정제 마스터가 있으면서 `PROCESSING`에 남은 사진은 0건이어야 하며, V12 적용 후 안전 조건이 누락된 공개 사진 자산은 0건이어야 한다. V13의 만료 원본 제약·정리 인덱스, V14의 제한된 후보 소유자 함수, V15의 `places_public_select`가 있어야 한다. 공개 모드에서는 전체 공개·게시 완료 기록에 연결된 장소만 읽을 수 있어야 한다.
...
- 本番PostgreSQLでV1〜V15履歴、アプリケーション14テーブル、session 2テーブルと`FORCE ROW LEVEL SECURITY`が13個の個人データテーブルへ適用されたことを確認する。V11適用後、完了job・検査通過sanitized masterがありながら`PROCESSING`に残る写真は0件で、V12適用後は安全条件が欠落した公開写真assetが0件でなければならない。V13の期限切れ原本制約・cleanup index、V14の限定候補owner関数、V15の`places_public_select`が存在し、public modeでは公開・配備済み記録へ接続されたplaceだけを読めなければならない。
