...
- 운영 PostgreSQL에서 V1~V8 이력과 `FORCE ROW LEVEL SECURITY`가 13개 개인 데이터 테이블에 적용됐는지 확인한다.
...
- 本番PostgreSQLでV1〜V8履歴と`FORCE ROW LEVEL SECURITY`が13個の個人データテーブルへ適用されたことを確認する。
