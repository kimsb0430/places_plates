...
- `main`의 `Verify`가 성공한 뒤 `Production smoke`가 실행되고 Vercel `/api/deployment`와 Cloud Run `/api/v1/health`의 `X-Places-Plates-Commit`이 병합 커밋과 같은지 확인한다. 자동 검사는 `/`, `/posts`, `/map`, 공개 목록·지도 API와 보안 헤더·비공개 Storage 경로 부재를 함께 검사한다. 운영 URL을 바꾸면 GitHub Repository Variables `PRODUCTION_FRONTEND_URL`, `PRODUCTION_API_URL`을 갱신하고, 실패 시 이전 Vercel 배포와 Cloud Run 리비전이 계속 트래픽을 받는지 확인한다.
...
- 키보드만 사용해 본문 바로가기, 홈의 여행 기록·지도 링크, 공개 목록·지도 카테고리 링크, 로그인, 사진 선택, 초안 편집·게시·삭제 버튼까지 이동할 수 있는지 확인한다. 공개 상세 사진 확대 화면은 Tab이 대화상자 밖으로 빠지지 않고 Escape로 닫을 때 호출 사진에 포커스가 돌아와야 한다. 모든 폼 컨트롤에는 3px 포커스 윤곽선이 보이고 빈 사진 설명은 제목 기반 대체 문구로 읽혀야 한다.
- 운영 관리 화면에서 테스트 초안과 테스트 공개 기록을 각각 삭제해 확인 문구, 목록 즉시 갱신, 공개 목록·지도·직접 상세 URL 제거를 확인한다. Storage 삭제 실패 시 API는 `503 POST_STORAGE_DELETE_FAILED`를 반환하고 DB 기록이 남아 재시도할 수 있어야 하며, 성공 시 해당 게시물의 `temporary/`·`sanitized/`·`variants/` 객체와 사진·업로드 행이 남지 않아야 한다.
...
- `main`の`Verify`成功後に`Production smoke`が実行され、Vercel `/api/deployment`とCloud Run `/api/v1/health`の`X-Places-Plates-Commit`がmerge commitと一致することを確認する。自動検査は`/`、`/posts`、`/map`、public list・map API、security header、private Storage path不存在を同時に検査する。Production URL変更時はGitHub Repository Variables `PRODUCTION_FRONTEND_URL`、`PRODUCTION_API_URL`を更新し、失敗時は以前のVercel deploymentとCloud Run revisionがtrafficを維持することを確認する。
