...
진행 상태: C34는 공개 목록·상세·장소 이력의 사진에 공통 우클릭·드래그·복사·모바일 길게 누르기 억제를 적용했다. 병합 후 운영 점검에서 Vercel `/_next/image`가 사용자 정의 보호 헤더를 보존하지 않는 것을 확인해 C34A post-merge fix로 같은 출처 `/api/public-images/**` Route Handler와 `unoptimized` 표시로 교체했다. Cloud Run 직접 사진과 Vercel 중계 사진이 동일 출처 리소스·프레임 차단 정책을 함께 반환해야 하며, 원본 미제공·메타데이터 제거·픽셀 워터마크가 실제 보호 경계다. C34A 운영 스모크 통과 후 다음은 C35 공개 응답과 전역 보안 헤더 강화다.
