...
진행 상태: C34는 공개 목록·상세·장소 이력의 사진에 공통 우클릭·드래그·복사·모바일 길게 누르기 억제를 적용했고 C34A에서 Vercel 같은 출처 이미지 중계와 운영 헤더 검사를 완료했다. C35는 Google Maps·Supabase 업로드 허용 목록을 유지한 전역 프런트 CSP, 브라우저 권한·프레임·MIME 보호 헤더와 Spring Boot API 전용 보안 헤더를 추가했다. 공개 목록·상세 JSON의 내부 Storage 키 비노출과 예약 정리 실패 로그의 메시지·스택 비기록을 회귀 테스트로 고정했다. 다음은 C36 지도 지연 로딩과 공개 이미지 성능 최적화다.
