...
진행 상태: C29는 공개 지도 API, 전체·맛집·여행지 URL 필터, 맛집 주황색 ‘맛’·여행지 초록색 ‘여’ Google 지도 마커와 명시적 지연 로딩을 구현했다. V16은 좌표가 연결된 기존 게시물의 지도 공개 상태를 보정하며 Google 좌표는 30일 유효 기간 안에서만 사용한다. C30은 공식 Supercluster 기반 확대 수준별 인접 마커 묶음, 포함 게시물 숫자와 클릭 확대를 구현했다. 다음은 C31 현재 지도 화면 경계의 게시물 합계다.
