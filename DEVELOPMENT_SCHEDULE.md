...
진행 상태: C34는 공개 목록·상세·장소 이력의 사진에 공통 우클릭·드래그·복사·모바일 길게 누르기 억제를 적용하고, Cloud Run 사진 API와 Vercel `/_next/image` 응답에 동일 출처 리소스·프레임 차단 정책을 추가했다. 원본 미제공·메타데이터 제거·픽셀 워터마크가 실제 보호 경계이며 보조 억제로 완전한 복사 방지를 주장하지 않는다. 다음은 C35 공개 응답과 전역 보안 헤더 강화다.
