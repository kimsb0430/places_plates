...
- 게시 완료 기록은 `/manage/posts/[postId]/edit`에서 소유자와 `PUBLISHED` 상태를 다시 확인한 뒤 공통 내용·카테고리 상세·장소·사진 순서·대표 사진·대체 텍스트를 수정한다. 공통 내용은 명시적인 저장 버튼을 사용하고, 사진과 장소 변경은 각각의 소유자 전용 관리 API로 저장한다. 새 사진 업로드 묶음은 새 초안을 만들지 않고 지정된 게시물에 연결한다.
- 다중 사진 업로드는 `photo/upload-concurrency.ts`의 2단계 큐를 사용한다. Supabase TUS 전송은 최대 2개로 제한하고 모든 전송이 끝난 뒤 Cloud Run 이미지 정제는 1개씩 실행한다. 진행률 요청도 25% 단위로 직렬화하며 실패 상태 저장을 기다린 뒤 재시도 버튼을 활성화해 업로드·재시도 경합을 줄인다.
...
- C40에서 홈은 하드코딩 목업을 제거하고 `GET /api/v1/public/posts`의 실제 합계와 최근 카드를 서버에서 읽는다. C40A는 헤더를 기록·지도 두 메뉴로 단순화하고 영웅 영역의 대표 게시물 사진 대신 `/manage` 기록 작성 카드를 배치한다. C40B는 홈 제목을 `나의 기록`으로 바꾸고 영웅 영역의 전체 기록·지도 링크와 기록 작성 카드의 설명을 모두 제거한다. C40C는 우측 영역 전체를 채우던 링크를 데스크톱 약 430×270px의 `기록하기` 버튼으로 줄여 하단 왼쪽에 배치하고 나머지는 배경 공백으로 유지하며, 모바일에서는 화면 폭에 맞춰 축소한다. 고정된 `Kyoto, Spring 2026` 하단 영역은 표시하지 않는다. 공개 상세의 `CATEGORY NOTE`는 사진보다 위에 배치하며, 갤러리 선택 시 원본 업로드가 아닌 최대 2,000px의 메타데이터 제거·워터마크 적용 `PUBLIC_DETAIL`을 키보드 닫기·포커스 복귀가 가능한 확대 대화상자로 표시한다.
...
- C39 배포 계약 검사는 Vercel 상태 route, Cloud Run health 헤더, Cloud Build 커밋 주입, `Verify` 성공 후 운영 스모크 연결이 함께 유지되는지 정적으로 확인한다. 실제 운영 스모크는 홈·목록·지도와 공개 API 상태·보안 헤더·비공개 경로 부재·병합 커밋 일치를 확인한다. C41의 `docs/OPERATIONS_RUNBOOK.md`는 RPO 24시간·RTO 4시간 초기 목표, DB와 Storage 분리 백업, 원본·세션 제외, 새 Supabase 프로젝트 복원, Vercel·Cloud Run 롤백과 장애·비용 대응을 단일 운영 절차로 정의한다. C42의 `config/production-guardrails.json`은 운영 도메인, 월 JPY 500 Google Cloud 알림 예산, Maps·Places 월 목표와 공급자 할당량 제약, Supabase Free·Vercel Hobby 사용량 기준을 기계 판독 가능한 계약으로 고정하며 `scripts/check-production-guardrails.ps1`이 루트 검증에서 확인한다. Cloud Build는 Cloud Run을 최소 0·최대 1 인스턴스로 배포한다.
- C43의 `config/release.json`은 제품·Semantic Version·annotated tag·운영 주소·최소 DB migration·watermark 정책·필수 `main` 검사를 단일 출시 계약으로 정의한다. `scripts/check-release-readiness.ps1`은 프런트 `package.json`, 백엔드 Gradle 버전, v1 출시 문서와 Release workflow의 일치를 루트·PR 검증에서 확인한다. `.github/workflows/release.yml`은 `v1.0.0` 태그 커밋이 `origin/main`에 포함되고 같은 SHA의 Secret protection·Verify·Production smoke와 실제 운영 스모크가 성공한 경우에만 `docs/releases/v1.0.0.md`로 GitHub Release를 생성한다.
...
- 초안·게시 기록 사진 편집 API는 `photos`의 기존 `display_order`, `is_cover`, `alt_text`를 사용한다. 배열 순서를 0부터 연속된 표시 순서로 저장하고, 대표 사진은 최대 한 장만 허용하며, 다른 소유자나 다른 기록의 사진 ID가 섞인 부분 갱신을 거부한다. 관리자 썸네일 응답은 `no-store`로 제공한다. 게시 기록 대상 업로드는 소유자 `PUBLISHED`와 카테고리 일치를 확인하고 기존 사진을 포함해 기록당 최대 100장을 허용한다.
...
운영 점검, DB·Storage 백업, 새 환경 복원, 배포 롤백과 장애 대응은 `docs/OPERATIONS_RUNBOOK.md`, 운영 도메인·예산·사용량 알림은 `docs/PRODUCTION_GUARDRAILS.md`, v1 출시 승인·태그·Release는 `docs/RELEASE_V1.md`를 기준으로 한다.
